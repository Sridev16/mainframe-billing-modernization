...
export class Data {
    ...
    public clone(): Data {
		...
		cloneData.padLeft = this.padLeft;
		cloneData.padRight = this.padRight;
        return cloneData;
    }
		...
		if (password) {
			...
			if (intensity === 'DRK') {
				...
				if (color) {
					...
					// When no color is explicitly specified, DSPATR(BL) won't take effect unless:
					// DSPATR(HI) is present AND DSPATR(CS) is NOT present
					if (highlight === 'BLINK' && (intensity !== 'high' || this.attributes.columnSeparator)) {
						highlight = '';
					}
				}
