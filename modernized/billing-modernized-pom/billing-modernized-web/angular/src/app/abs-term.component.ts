...
import { Attributes, LockScreenKeyboard } from './term/message';
...
export abstract class AbstTermComponent implements AfterViewInit, OnDestroy, AfterViewChecked {
    ...
    abstract getIdAnchor(): HTMLElement;
    ...
    public ngAfterViewChecked(): void {
        ...
        if (this.haveToChangeTargetAccessibility()) {
            ...
            this.getIdAnchor().setAttribute("style", "pointer-events: none");

            // Keep accessible as target usefull fields
            let msgs : HTMLElement[] = [];
            let numberLine : number = 0;
            this.childrenLevel(this.getIdAnchor(), 0).forEach( (c: HTMLElement) => {
                msgs.push(c);
                numberLine = Math.max(numberLine, this.childrenLevel(c, 1).length);
            });
            ...
            for (let i = 0; i<numberLine; i++) {
                ...
                for (let j = 0; j<msgs.length; j++) {
                    let divLine = this.childrenLevel(msgs[j], 1)[i];
                    if (divLine && this.childrenLevel(divLine, 0).filter( c => !c.classList.contains("lgr_00")).length > 0) {
                ...
                if (retainDivLine != null) {
                    retainDivLine.setAttribute("style", "pointer-events: visible");
                }
            }
        } else {
            // Re-establish default behaviour
            this.getIdAnchor().removeAttribute("style");
        }
    ...
    protected haveToChangeTargetAccessibility(): boolean {
        ...
        if (this.childrenLevel(this.getIdAnchor(), 0).length > 1 && this.childrenLevel(this.getIdAnchor(), 1).filter( c => c.style.position === "absolute").length > 0) {
            return true;
        }
    ...
    private animate():void {
        document.getElementById("idFooter").classList.remove("run-animation");
        setTimeout(() => {
            document.getElementById("idFooter").classList.add("run-animation");
        }, 10);
    }
    ...
    private insertComponents(map: Map, componentType: any) {
        ...
        // Do not reuse existing component if component uses SLNO(*VAR)
        if(existingComponent !== undefined && !map.slnoVar){
    ...
    private decodeFields(map: any, inputMap: any): void {
        ...
        // Component with SLNO(*VAR) also behaves like overlay
        if(inputMap.slnoVar){
        	this.mapFieldToData( map, 'overlay', true);   
        }
	                    ...
	                    if (Array.isArray(data)) {
	                    	...
	                    	if(Object.keys(data).length !== 0) {
		                    	...
		                    	if (data.isModified()) {
                                    ...
                                    let isIso = false;
                                    let controlCharacters = data.attributes.controlCharacters;
                                    

		                        	let fieldState: any = {"component": componentName, 'id': fieldName, 'value': value, valueChanged, hasInitialValue, isIso, controlCharacters, hasLowValues, ...(data.attributes !== undefined && data.attributes.isPassword && {'ispassword': data.attributes.isPassword})};
		                        	termMessage.fields.push(fieldState);
		                    	}
		                    }
		                }
    ...
    */
    ...
    private getActiveComponent(activeElement: Element): string {
        ...
        return this.getDefaultActiveComponent();
    }

    private getDefaultActiveComponent(): string {
        let components = Object.keys(this.termState.componentsByName);
        
        if (this.termState.injectedModalComponents.length > 0) {
            for (let comp of components) {
                if (comp !== 'standard-messageline' && comp !== 'standard-arraymessageline' && comp.startsWith('modal-')) {
                    return comp;
                }
            }
        }
        ...
        for (let comp of components) {
            if (comp !== 'standard-messageline' && comp !== 'standard-arraymessageline') {
                return comp;
            }
        }
   ...
   */
   ...
   /**
    ...
    // Custom method to replace jQuery children() functionality
    /**
     * Returns all the children of an HTML element of a given level
     *
     * @param element   - the HTML element we want to get the children
     * @param level     - the level for which we want to get all the children
     * @returns         - the array of children HTML elements at the level
     */
    public childrenLevel(element: HTMLElement, level: number): HTMLElement[] {
        const res: HTMLElement[] = [];
        const queue = [];
        queue.push([element, -1]);
        while (queue.length > 0) {
            const tmp = queue.shift();
            const child: HTMLElement = tmp[0];
            const curLevel = tmp[1];
            if (curLevel < level) {
                const children = Array.from(child.children);
                children.forEach((element: HTMLElement) => {
                    queue.push([element, curLevel+1]);
                });
            }
