...
export abstract class MaskFieldComponent extends CursorDynamicFieldComponent {
	...
	/**
	 * Get the edit word value for fallback in dynamic mask building.
	 * Overridden by subclasses that define edtwrd as @Input.
	 * @protected
	 */
	protected getEdtwrd(): string {
	    return null;
	}
    /**
    ...
    private buildDynamicMask(value: string) {
	    ...
	    for (let index = 0; index < this.mask.length; index++) {
	        if (this.isProtectedChar(this.mask[index])) {
	            let displayChar = value.charAt(index);
	            let edtwrd = this.getEdtwrd();
	            if (displayChar === this.maskChar
	                && edtwrd
	                && index < edtwrd.length
	                && edtwrd.charAt(index) !== this.maskChar) {
	                displayChar = edtwrd.charAt(index);
	            }
	            if (displayChar !== this.maskChar) {
	                if (lastAdded !== index) {
