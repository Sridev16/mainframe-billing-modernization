...
export class NumericalService {
	...
	static isValidNumericFormat(value: string, numericProperties: NumericProperties): boolean {
		...
		if (split[0].length > 16) {
			noLeadingZeroes = BigInt(split[0]);
			wholePart = isNaN(Number(noLeadingZeroes)) ? 0 :  noLeadingZeroes.toString().length;
		}
		else {
			noLeadingZeroes = parseInt(split[0], 10);
			wholePart = isNaN(noLeadingZeroes) ? 0 :  noLeadingZeroes.toString().length;
		}
