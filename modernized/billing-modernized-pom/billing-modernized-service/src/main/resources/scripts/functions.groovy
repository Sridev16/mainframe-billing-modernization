...
def execStep (String stepName, String programName, Map programResults, Closure exec) {
	try {
	  ...
	  def groovyExecutionResult = null
	  if(programResult instanceof ProgramExecutionResult){
		pge = (ProgramExecutionResult) programResult
		if(pge.getErrorMessage() != null) {
			groovyExecutionResult = new GroovyExecutionResult(stepName, programName, programResult.getReturnCode(), [(GroovyExecutionException.ERROR_MESSAGE): pge.getErrorMessage()])
		}
		else {
			groovyExecutionResult = new GroovyExecutionResult(stepName, programName, programResult.getReturnCode())
		}
	  }
	  else {
		groovyExecutionResult = new GroovyExecutionResult(stepName, programName, programResult.getReturnCode())
